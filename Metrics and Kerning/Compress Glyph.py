# MenuTitle: Compress Glyph (Manuel) 
# -*- coding: utf-8 -*-
...
class CompressGlyph(mekkaObject):
	...
	def update(self, sender=None):
		...
		names.extend(sorted(set([f'*{g.name[g.name.find("."):]}' for g in Glyphs.font.glyphs if "." in g.name and not str(g.name)[0] == (".")])))
					...
					for deletePair in deletePairs:
						try:
							...
							thisFont.removeKerningForPair(
								masterID,
								leftKey if str(leftKey)[0] == ("@") else thisFont.glyphForId_(leftKey).name,
								rightKey if str(rightKey)[0] == ("@") else thisFont.glyphForId_(rightKey).name,
							)
						except Exception as e:  # noqa: F841
