#MenuTitle: Copy Kerning Pairs Custom ...
# -*- coding: utf-8 -*-
...
class CopyKerningPairs( object ):
	def __init__( self ):
		...
		# Common:
		self.w.allMaster = vanilla.CheckBox((spaceX, -20-15, 100, -15), "All masters", sizeStyle='regular', value=True)
		...
		self.w.open()
		tab2.radio.set(True)
		tab2.popNum1.enable(True)
		tab2.popNum2.enable(True)
		self.w.makeKey()
	...
	def checkField(self, sender):
		try:
			...
			print("Copy kerning Pairs Error (checkField): {}".format(e))
	...
	def checkRadio(self, sender):
		try:
			...
			Glyphs.showMacroWindow()
			print("Copy kerning Pairs Error (checkRadio): {}".format(e))
	...
	def applyKern1(self, theMaster, newKernDic, L0, R0, L1, R1):
		...
		for i in range(len(pairList)):
			if pairList[i][1] == R0:
				print("\t{},  {},  {}".format(pairList[i][0], R1, pairList[i][2]))
	...
	def applyKern2(self, theMaster, newKernDic, L0, R0, L1, R1):
		...
		for i in range(len(pairList)):
			if pairList[i][0] == L0:
				print("\t{},  {},  {}".format(L1, pairList[i][1], pairList[i][2]))
	...
	def applyKern3 (self, theMaster, newKernDic, L0, R0, L1, R1):
		...
		while i != len(newKernDic[theMaster.id]):
			if pairList[i][0] == L0 and pairList[i][1] == R0:
				value = pairList[i][2]
				#print("\t{},  {},  {}".format((pairList[i][0], pairList[i][1], pairList[i][2]))
				break
			i =i+1
		if value == None:
			print("The source pair does not exist.")
		else:
			print("\t{},  {},  {}".format(L1, R1, pairList[i][2]))
	...
	def dupliKernPair(self, newKernDic, L0, R0, L1, R1):
		try:
			...
			Glyphs.showMacroWindow()
			print("Copy kerning Pairs Error (dupliKernPair): {}".format(e))
					...
					for i in range(len(pairList)):
						...
						if pairList[i][0] == keyL and pairList[i][1] == keyR:
							...
							if int(abs(float(pairList[i][2])*scale)) >= int(skip):
						 		...
						 		print("\t{},  {},  {}".format(dicL[keyL], dicR[keyR], theValue))
						 		#return "\t{},  {},  {}".format(dicL[keyL], dicR[keyR], theValue)
						 		# return "impossible to return anything here. no clue why. the script just stops working, even without any error message."

		except Exception as e:
			Glyphs.showMacroWindow()
			print("Copy kerning Pairs Error (applyKernPreset): {}".format(e))
	...
	def dupliKernPreset(self, newKernDic, dic):
		...
		try:
			...
			print('dic----------')
			print(dic)
			for key, value in dic.items(): #dict.items() didn't work anymore (python 3?)
				if f.glyphs[key]:
					print(f.glyphs[key])
			...
			#reportList.append (newline)
			#reportText = ', '.join(reportList)
			#reportText += '\n' + newLine 
			reportText += '\n{}'.format(newLine)
			#print(reportText)
			...
			Glyphs.showMacroWindow()
			print("Copy kerning Pairs Error (dupliKernPreset): {}".format(e))
	...
	def CopyKerningPairsMain( self, sender ):
		try:
			...
			# T A B 1 - P A I R  O P E R A T I O N

			if self.w.tabs.get()==0: # If it's an pair operation
				print("Pair Operation")
			...
			# T A B 2 - P R E S E T
				...
				if self.w.tabs[1].radio.get()==0: # If it's Letter preset
					...
					# Caps & Lowercase to Superscript

					else:
						print("# Caps & Lowercase to Superscript")
						...
						#smallLetterDic = (letterDic.items() + self.miscSymbolDic(miscType).items()) #(error: + operator is seemingly not working anymore in python 3)
						#smallLetterDic = (letterDic.items() | self.miscSymbolDic(miscType).items())
						smallLetterDic = (letterDic | self.miscSymbolDic(miscType))
			...
			Glyphs.showMacroWindow()
			print("Copy Kerning Pairs Error: {}".format(e))
